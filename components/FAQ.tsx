...
import { ChevronDown, Search, HelpCircle, CornerDownRight, X, ThumbsUp, ThumbsDown } from "lucide-react";
...
function TechChip({ name, tech }: { name: string; tech: { color: string; url: string; icon?: string } }) {
    return (
        ...
        >
            {tech.icon ? (
                <img src={tech.icon} alt={`${name} logo`} className="size-[11px] shrink-0 object-contain" />
            ) : (
                <span
...
function HelpfulFeedback({ questionIndex }: { questionIndex: number }) {
    ...
    if (feedback) {
        return (
            <motion.div
                ...
                className="flex items-center gap-1.5 text-[10px] text-zinc-600 mt-2"
            >
                <span className="text-zinc-500">Thanks for the feedback!</span>
            </motion.div>
        );
    }
        ...
        <div className="flex items-center gap-2 mt-2 pt-2 border-t border-white/[0.04]">
            ...
            <div className="flex gap-1">
                <motion.button
                    ...
                    className="p-1 rounded-md text-zinc-600 hover:text-green-400 hover:bg-green-400/10 transition-colors"
...
function FAQItem({ faq, isOpen, onToggle, index }: { faq: typeof faqs[0]; isOpen: boolean; onToggle: () => void; index: number }) {
    ...
    return (
        ...
        >
            ...
            {/* Grid-rows accordion (CSS-only height animation) */}
            <div
                className="grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.23,1,0.32,1)]"
                style={{ gridTemplateRows: isOpen ? '1fr' : '0fr' }}
            >
                <div className="overflow-hidden">
                    <div id={answerId} role="region" className="px-4 pb-4 text-zinc-400 text-[13px] md:text-xs leading-relaxed border-t border-white/[0.04] pt-3 max-w-4xl">
                        <div className="flex gap-2.5 items-start">
                            <CornerDownRight className="w-3.5 h-3.5 mt-[3px] shrink-0 text-zinc-600" />
                            <div className="flex-1">
                                {renderWithTechChips(faq.answer)}
                                <HelpfulFeedback questionIndex={index} />
                            </div>
                        </div>
                    </div>
                </div>
...
function CategoryCount({ category, isActive }: { category: string; isActive: boolean }) {
    ...
    return (
        <motion.span
            ...
            className={`relative z-10 flex items-center justify-center w-4 h-4 rounded-full text-[8px] font-bold ${isActive
                ? "bg-white text-black"
...
export function FAQ() {
    ...
    const [prevCategory, setPrevCategory] = useState("General");
        ...
        <section className="py-16 bg-black relative" id="faq">
            ...
            <div className="max-w-4xl mx-auto px-4 relative z-10">
                ...
                >
                    ...
                    >
                        {searchQuery ? (
                            <X className="w-3.5 h-3.5 text-zinc-600 group-focus-within/search:text-white transition-colors duration-200" />
                        ) : (
                            <Search className="w-3.5 h-3.5 text-zinc-600 group-focus-within/search:text-white transition-colors duration-200" />
                        )}
                    </motion.div>
                    <input
                        type="text"
                ...
                {/* Category tabs with count bounce */}
                <motion.div
                    initial={{ opacity: 0, y: 8 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                ...
                >
                    {categories.map((category) => (
                        ...
                        >
                            ...
                            <CategoryCount category={category} isActive={activeCategory === category} />
                        </motion.button>
                    ))}
                </motion.div>

                {/* FAQ list with grid-rows accordion */}
